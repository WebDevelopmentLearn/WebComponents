document.addEventListener("DOMContentLoaded", () => {
  ...
  signUpForm.addEventListener("submit", (event) => {
    ...
    if (isEmailAlready(allUsers, currentUserData.email)) {
      ...
      const validArr = [];
      for (const key in allValid) {
        const value = allValid[key];
        console.log(`key: ${key}  value: ${value}`);
        validArr.push(value);
      }
      const isValid = validArr.every((el) => {
        return el;
      });
      if (isValid) {
        ...
        alert("Не все поля прошли валидацию");
      }
    }
  });
  ...
  phoneNumberInput.addEventListener("keyup", (e) => {
    ...
    responseStatus(isValid ? "success" : "fail", statusMsg, message);
    if (isValid) {
      allValid.phoneNumberIsVald = true;
    }
  });
